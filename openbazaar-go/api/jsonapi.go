...
//add by ChenYang 2018-02-26 19:08:00
func (i *jsonAPIHandler) POSTAdd(w http.ResponseWriter, r *http.Request) {
	...
	fileHash, err1 := ipfs.AddFile(i.node.Context, fileInfo.FilePath)
	...
	fmt.Println("add file successfully file hash = ", fileHash)

	jsonHashes, err := json.MarshalIndent(fileHash, "", "    ")
	if err != nil {
		ErrorResponse(w, http.StatusInternalServerError, err.Error())
		return
	}
...
//added by ChenYang 2018-02-27 10:48:00 start
func (i *jsonAPIHandler) GETContent(w http.ResponseWriter, r *http.Request) {
	_, fileHash := path.Split(r.URL.Path)
	ctx, cancel := context.WithTimeout(context.Background(), time.Minute*2)
	defer cancel()
	dr, err := coreunix.Cat(ctx, i.node.IpfsNode, "/ipfs/"+fileHash)
	fmt.Println("cat file hash is ", fileHash)
	if err != nil {
		ErrorResponse(w, http.StatusInternalServerError, err.Error())
		return
	}
	...
	http.ServeContent(w, r, fileHash, time.Now(), dr)
}
...
func (i *jsonAPIHandler) GETFile(w http.ResponseWriter, r *http.Request) {
	...
	http.ServeContent(w, r, fileHash, time.Now(), strings.NewReader("Get file ok"))
}

//added by ChenYang 2018-02-27 10:48:00 end
